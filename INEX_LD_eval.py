...
class Evaluator:
    def __init__(self, conf=None):
        ...
        self.answers = self.load('INEX_LD_answers.pkl')
        self._vocab = self.load('INEX_LD_vocabulary.pkl')
    ...
    def save_epoch(self, model, epoch):
        if not os.path.exists('models/INEX_LD/'):
            os.makedirs('models/INEX_LD/')
        model.save_weights('models/INEX_LD/weights_epoch_%d.h5' % epoch, overwrite=True)

    def load_epoch(self, model, epoch):
        assert os.path.exists('models/INEX_LD/weights_epoch_%d.h5' % epoch), 'Weights at epoch %d not found' % epoch
        model.load_weights('models/INEX_LD/weights_epoch_%d.h5' % epoch)
    ...
    def train(self, model):
        ...
        dataset = self.load('INEX_LD_train.pkl')
        test_set = self.load('INEX_LD_test.pkl')
        query2id = self.load('INEX_LD_query2id.pkl')
        ...
        self._eval_sets = dict([('INEX_LD', eval_sets)])
        ...
        for i in range(1, nb_epoch+1):
            ...
            bad_answers = self.pada(random.sample(self.answers.values() * 2, len(good_answers)))
    ...
    def eval_sets(self):
        if self._eval_sets is None:
            self._eval_sets = dict([(s, self.load(s)) for s in ['INEX_LD_train.pkl']])
...
if __name__ == '__main__':
    ...
    conf = {
        'question_len': 10,
        'answer_len': 10,
        'n_words': 14163,  # len(vocabulary) + 1
        'margin': 0.05,
        ...
        'model_params': {
            ...
            'initial_embed_weights': np.load('embeddings/INEX_LD_300_dim.embeddings'),
