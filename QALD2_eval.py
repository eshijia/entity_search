...
class Evaluator:
    def __init__(self, conf=None):
        ...
        self.answers = self.load('QALD2_answers.pkl')
        self._vocab = self.load('QALD2_vocabulary.pkl')
    ...
    def save_epoch(self, model, epoch):
        if not os.path.exists('models/QALD2/'):
            os.makedirs('models/QALD2/')
        model.save_weights('models/QALD2/weights_epoch_%d.h5' % epoch, overwrite=True)

    def load_epoch(self, model, epoch):
        assert os.path.exists('models/QALD2/weights_epoch_%d.h5' % epoch), 'Weights at epoch %d not found' % epoch
        model.load_weights('models/QALD2/weights_epoch_%d.h5' % epoch)
    ...
    def train(self, model):
        ...
        dataset = self.load('QALD2_train.pkl')
        test_set = self.load('QALD2_test.pkl')
        query2id = self.load('QALD2_query2id.pkl')
        ...
        self._eval_sets = dict([('QALD2', eval_sets)])
    ...
    def eval_sets(self):
        if self._eval_sets is None:
            self._eval_sets = dict([(s, self.load(s)) for s in ['QALD2_train.pkl']])
...
if __name__ == '__main__':
    ...
    conf = {
        'question_len': 10,
        'answer_len': 10,
        'n_words': 20213,  # len(vocabulary) + 1
        'margin': 0.05,
        ...
        'model_params': {
            ...
            'initial_embed_weights': np.load('embeddings/QALD2_300_dim.embeddings'),
