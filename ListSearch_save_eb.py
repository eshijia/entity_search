...
class Evaluator:
    ...
    def save_epoch(self, model, epoch):
        if not os.path.exists('models/ListSearch/'):
            os.makedirs('models/ListSearch/')
        model.save_weights('models/ListSearch/weights_epoch_%d.h5' % epoch, overwrite=True)

    def load_epoch(self, model, epoch):
        assert os.path.exists('models/ListSearch/weights_epoch_%d.h5' % epoch), 'Weights at epoch %d not found' % epoch
        model.load_weights('models/ListSearch/weights_epoch_%d.h5' % epoch)
    ...
    def eval_sets(self):
        if self._eval_sets is None:
            self._eval_sets = dict([(s, self.load(s)) for s in ['ListSearch_train.pkl']])
...
if __name__ == '__main__':
    ...
    conf = {
        'question_len': 10,
        'answer_len': 10,
        'n_words': 15062,  # len(vocabulary) + 1
        'margin': 0.05,
        ...
        'model_params': {
            ...
            'initial_embed_weights': np.load('embeddings/ListSearch_300_dim.embeddings'),
    ...
    np.save('models/ListSearch_embeddings.npy', weights)
